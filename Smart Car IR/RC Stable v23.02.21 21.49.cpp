...
void underControl() {
  if (Previous == increment || Previous == 0x40BFBA45) {
    flash();
    if (holdFor <= 500) {
      holdFor += 20;
    }
  ...
  else if (Previous == decrement || Previous == 0x40BF9A65) {
    flash();
    if (holdFor >= 60) {
      holdFor -= 20;
    }
  ...
  else if (Previous == rightCode || Previous == 0x40BF20DF) {
    rot90c();
  }
  else if (Previous == leftCode || Previous == 0x40BFE21D
          ) {
    rot90cc();
  }
  else if (Previous == upCode || Previous == 0x40BF12ED) {
    accelerate();
  }
  else if (Previous == downCode || Previous == 0x40BF02FD) {
    reverse();
  }
  else if (Previous == nutralCode || Previous == 0x40BF22DD) {
    falseEach();
    nutral();
  }
  else if (Previous == spinCodeC || Previous == 0x40BF58A7) {
    nutral();
    falseEach();
    flash();
    spinc();
  }
  else if (Previous == spinCodeCC || Previous == 0x40BF5AA5) {
    nutral();
    falseEach();
    flash();
    spincc();
  }
  else if (Previous == zzCode || Previous == 0x40BF50AF) {
    nutral();
    falseEach();
    flash();
    zzMode = true;
  }
  else if (Previous == blinkerCode || Previous == 0x40BF7887) {
    nutral();
    falseEach();
    flash();
    blinkerMode = true;
...
void loop() {
  ...
  if (receiver.decode(&results)) {
    ...
    if (Previous == trigCode || Previous == 0x40BFA05F) {
      if (RCmode == false) {
        lightsOff();
        analogWrite(YLed, 255);
        RCmode = true;
